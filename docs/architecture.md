...
**Still true, and still a deliberate choice, not an oversight:** no background telemetry in the free-forever core — nothing is collected from usage, ever.

**The one deliberate, narrow exception (added 0.4):** a one-time, two-question install survey ("how did you hear about this" + "experience 1–5"). Its contract, enforced by `packages/cli/src/commands/survey.ts` and its tests: it only ever *sends* what the user explicitly typed, plus the CLI version — skipping sends zero bytes; it is offered at most once per machine, never in CI, never without a TTY, and `VOVY_NO_SURVEY=1` suppresses it entirely; the payload has no machine, project, or identifying information. It writes to an INSERT-only table (the embedded key can never read data back — verified against the live endpoint, not assumed). This exists because npm download counts are the only signal an npm package gets, and a volunteered two-question answer is the cheapest honest alternative to actual telemetry, which stays off the table. The MCP server and every MCP tool remain fully network-free.
