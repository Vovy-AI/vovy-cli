...
`detect()` for Cline always returns `false` — there's no reliable filesystem signal that Cline (a VS Code extension) is installed, so it's only ever targeted via `vovy install --host cline`, never silently.
